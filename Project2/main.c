...
int main(int arg, char* args[]) {
    ...
    general_reg.reg[$sp] = 0x10000000;

// Start Single Cycle
    // 탈출 조건 명시 $ra 값이 -1이 되면 종료, $31번에 탈출 주소가 명시되어 있음
    while(0xFFFFFFFF == general_reg.reg[$ra]) {
    // fetch instruction
    ...
    // TODO : Wrap it all of the decoded step into single function
        ...
        CU_opcode cu_opcode = {decoded.opcode, decoded.funct};
    ...
    // Branch and Jump Processing
        ...
        uint32_t isBranch = alu_output.isZero & control.branch;

    //PIPLINE ID LATCH ACCESS
        /* some of the code */
    ...
    // TODO: wrap it.
    // write_back();
        set_register_with_write_data(data_path_to_register, control.reg_write);
